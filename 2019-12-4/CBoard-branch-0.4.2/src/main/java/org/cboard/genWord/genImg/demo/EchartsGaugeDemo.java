...
import static org.cboard.genWord.genImg.utils.BigDecimalUtils.genFormatterStr2;
import static org.cboard.genWord.genImg.utils.BigDecimalUtils.getRateFromNum;
...
//相应的formatter
            ...
            dataStyle.textStyle().fontSize(15);
            ...
            if(i==j){
                g1.radius(getRateFromNum(0.40));
            }
            else{
                g1.radius(getRateFromNum(0.40-0.15*abs(1-i)));
            }

            if(i!=1) g1.center(getRateFromNum(0.175+i*0.325),getRateFromNum(0.5));
            //g1.radius(100+i*80);

            //根据传入的参数值 的大小 动态修改相应的仪表盘的 单位大小,需要将相应的索引值 传入，针对 每一个仪表盘单独进行修改
            String formatter2=genFormatterStr2(Values,temp,i);

            // System.out.println("old value ="+((Data)temp[i]).getValue()+"");

            g1.axisLine().lineStyle().width(10);
     ...
     * */
    public static String genGaugeConmen_union_all(String titile,String subtext,String formatter,Object[]gaugeDatas,Object[] Values){
