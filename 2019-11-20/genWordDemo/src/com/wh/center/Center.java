...
public class Center extends WordTemplete{
    ...
    public static String genDepartReport(String[]pngArray,ReportEntity reportEntity) throws DocumentException, IOException{
        ...
        fileName="d:\\dome_center_"+ UUID.randomUUID().toString()+".doc";
        wt.openDocument(fileName);
        wt.insertTitle("分行业绩指数报告(总行版)", fontSize, Font.BOLD, Element.ALIGN_CENTER);

        wt.insertContext("中国建设银行"+reportEntity.getYear()+"年度业绩总收入为（"+reportEntity.getYearIncomeAll()+"），同期增长（"+reportEntity.getIncrease()+"）%，各项收入来源如下:", fontSize, Font.NORMAL, Element.ALIGN_LEFT);
        wt.insertImg(sourcePath[0], Image.ALIGN_CENTER, 12, 35, 50, 50, 50, 30);
        ...
        wt.insertImg("earth.png", Image.ALIGN_CENTER, 12, 35, 50, 50, 50, 30);
        wt.insertContext("年度各分行资金沉淀收入收入为:", fontSize, Font.NORMAL, Element.ALIGN_LEFT);
        wt.insertImg("earth.png", Image.ALIGN_CENTER, 12, 35, 50, 50, 50, 30);


        wt.insertContext("月度明细如下：\n", fontSize, Font.NORMAL, Element.ALIGN_LEFT);
        wt.insertImg(sourcePath[1], Image.ALIGN_CENTER, 12, 35, 50, 50, 50, 30);

        wt.insertContext("<全行托管费用收入月度情况分析>", fontSize, Font.NORMAL, Element.ALIGN_LEFT);
        wt.insertImg(sourcePath[2], Image.ALIGN_CENTER, 12, 35, 50, 50, 50, 30);

        wt.insertContext("<全行银行费用收入月度情况分析>", fontSize, Font.NORMAL, Element.ALIGN_LEFT);
        wt.insertImg(sourcePath[3], Image.ALIGN_CENTER, 12, 35, 50, 50, 50, 30);

        wt.insertContext("<全行资金沉淀收入月度情况分析>", fontSize, Font.NORMAL, Element.ALIGN_LEFT);
        wt.insertImg(sourcePath[4], Image.ALIGN_CENTER, 12, 35, 50, 50, 50, 30);

        wt.insertContext("<全行客户数月度情况分析>", fontSize, Font.NORMAL, Element.ALIGN_LEFT);
        wt.insertImg(sourcePath[5], Image.ALIGN_CENTER, 12, 35, 50, 50, 50, 30);

        wt.insertContext("<全行产品数量月度情况分析>", fontSize, Font.NORMAL, Element.ALIGN_LEFT);
        wt.insertImg(sourcePath[6], Image.ALIGN_CENTER, 12, 35, 50, 50, 50, 30);


        wt.insertContext("<全行资产组合财务净值规模月度情况分析>", fontSize, Font.NORMAL, Element.ALIGN_LEFT);
        wt.insertImg(sourcePath[7], Image.ALIGN_CENTER, 12, 35, 50, 50, 50, 30);
        ...
        wt.insertImg(sourcePath[8], Image.ALIGN_CENTER, 12, 35, 50, 50, 50, 30);

        wt.insertContext("<分行托管费收入贡献分析>", fontSize, Font.NORMAL, Element.ALIGN_LEFT);
        wt.insertImg(sourcePath[9], Image.ALIGN_CENTER, 12, 35, 50, 50, 50, 30);

        wt.insertContext("<分行银行费收入贡献分析>", fontSize, Font.NORMAL, Element.ALIGN_LEFT);
        wt.insertImg(sourcePath[10], Image.ALIGN_CENTER, 12, 35, 50, 50, 50, 30);

        wt.insertContext("<分行资金沉淀收入贡献分析>", fontSize, Font.NORMAL, Element.ALIGN_LEFT);
        wt.insertImg(sourcePath[11], Image.ALIGN_CENTER, 12, 35, 50, 50, 50, 30);

        wt.insertContext("<分行客户数贡献分析>", fontSize, Font.NORMAL, Element.ALIGN_LEFT);
        wt.insertImg(sourcePath[12], Image.ALIGN_CENTER, 12, 35, 50, 50, 50, 30);

        wt.insertContext("<分行产品数贡献分析>", fontSize, Font.NORMAL, Element.ALIGN_LEFT);
        wt.insertImg(sourcePath[13], Image.ALIGN_CENTER, 12, 35, 50, 50, 50, 30);

        wt.insertContext("<分行资产组合财务净值贡献分析>", fontSize, Font.NORMAL, Element.ALIGN_LEFT);

        wt.insertImg(sourcePath[14], Image.ALIGN_CENTER, 12, 35, 50, 50, 50, 30);
...
//        wt.insertRiskControlTable();
    ...
    public static void main(String[] args) throws DocumentException, IOException {
        ...
        genDepartReport(sourcePath,reportEntity1);
