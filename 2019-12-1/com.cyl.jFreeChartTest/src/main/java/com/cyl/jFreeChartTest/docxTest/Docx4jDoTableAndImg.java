...
/**
 * DocxDoTable docx4j 在doc中指定的表格模板地方，动态向下拼装tr && 向word指定书签处、指定文字处中插入图片
 * 
 * @author caoyulun
 *
 */
public class Docx4jDoTableAndImg {
	...
	public static void main(String[] args) throws Exception {
		String templatefile = "E:\\bigData\\workSystemFile\\workSystemFile\\demo4.docx";
		String target = "E:\\bigData\\workSystemFile\\workSystemFile\\wh_target" + System.currentTimeMillis() + ".docx";
		
		WordprocessingMLPackage template = WordTemplate.getTemplate(templatefile);
		// -------------------------替换文字----------------------------------
		Map<String,String> markTxtValMap = new HashMap<String,String>();
		markTxtValMap.put("c_port_name", "社保101");
		markTxtValMap.put("c_born_date", "1999-09-09");
		replaceText(template, markTxtValMap);
		...
		// 根据特殊标签获取要修改的表格
		Tbl tempTable = WordTemplate.getTemplateTable(tables, "行业类别");
		JSONArray jsonArray = JSONArray.parseArray(JSON.toJSONString(demoList));
		addDocTableRows(tempTable, 3, jsonArray, true, totalDataJson);
		...
		addImgOnTextPosition(template, markAndImgroute);
