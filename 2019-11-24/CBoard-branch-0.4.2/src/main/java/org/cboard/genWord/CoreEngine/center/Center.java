...
public class Center extends WordTemplete {
    ...
    public static String genCenterReport(String[]pngArray,ReportEntity reportEntity) throws DocumentException, IOException{
        ...
        wt.insertContext("分行业绩分析如下：", fontSize, Font.NORMAL, Element.ALIGN_LEFT);
        ...
        wt.insertContext("分行业绩指标计算规则为：100*托管费用收入排名百分比+100*银行费用收入排名百分比+100*资金沉淀收益排名百分比+100*客户数量排名百分比+100*产品数量排名百分比+100*资产规模排名百分比+100*建行规模占比排名百分比+100*分行资源投入排名百分比", fontSize, Font.NORMAL, Element.ALIGN_LEFT);
...
//        }
        System.out.println(genCenterReport(getImgFileNameArrayByCenter(),reportEntity1));
